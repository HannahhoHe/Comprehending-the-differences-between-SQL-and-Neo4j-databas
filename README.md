...
The link above is a dataset called “citations” from the Neo4j Sandbox. The data contains the information about books – book title, authors, citations, etc. In the case below, I use Python to run Cypher queries to retrieve any authors who have never worked with Peter G. Neumann but with his coauthors. The candidates were listed in an order with the most collaborations on the top.   
```
query = """
